...
export default function Checkout() {
  ...
  return (
    ...
    <TextField variant='standard' label=" Current Address" fullWidth/>
    </Box>
    <Box sx={{margin:"auto" , marginTop:"20px",width:"100%", display:"flex", justifyContent:"space-between" ,flexWrap:"wrap"}}>
    <TextField variant='standard' label="City"/>
    <TextField variant='standard' label="State"/>
    </Box>
    <Box sx={{margin:"auto" , marginTop:"20px",width:"100%", display:"flex", justifyContent:"space-between" ,flexWrap:"wrap"}}>
    ...
    <Box sx={{width:"50%"}}>
      <Typography sx={{textAlign:'center'}}>Current Order Status (Shipping Free)</Typography>
       <Box sx={{overflow:"scroll" , width:"100%" , height:"65%"}}>
       {data.map((el, index) => {
           {total+=Number(el.price*el.quantity)}
           return <div key={index} className="flexdiv">
           <div className="imgdiv">
             {" "}
             <img id='cartimage' src={el.imgUrl} alt="" />
           </div>
